<script>
	...
	$effect(() => {
		...
		const query =
			results["search-results"]["opensearch:Query"]["@searchTerms"];
		...
		(async () => {
			for (let i = 0; i < status.length; i++) {
				...
				const included =
					result["search-results"]["opensearch:totalResults"] > 0
						? 1
						: 0;
				status = status.map((s, idx) =>
					idx === i ? { ...s, Status: included } : s,
				);
			}
		})();
	});
	...
	function iconClass(title) {
		const paper = status.find((d) => d["Title"] === title);
		if (paper?.["Status"] === 1) return "found";
		if (paper?.["Status"] === 0) return "unfound";
		return "unknown";
	}

	const total = $derived(seeds.length);
	const checked = $derived(
		status.filter((d) => d["Status"] !== undefined).length,
	);
	...
	{#if items.length > 0}
		<div class="seed-papers">
			...
			{#each list(name, items) as e (e["Title"])}
				<span class="paper">
					<span class={"icon " + iconClass(e["Title"])}>
						{icon(e["Title"])}
					</span>
					<a
						class="block"
