<script>
	...
	function handleSelect(keyword, category) {
		const newData = data
			...
			.filter(
				(d) => !(d["Keyword"] === "" && d["Category"] === category),
			);
	...
	function handleDeselect(keyword, category) {
		onquerychange(
			data.filter(
				(d) =>
					!(d["Keyword"] === keyword && d["Category"] === category),
			),
		);
	}
	...
	function handleCategoryEdit(oldValue, newValue) {
		onquerychange(
			data.map((d) =>
				d["Category"] === oldValue
					? { Category: newValue, Keyword: d["Keyword"] }
					: d,
			),
		);
	}
	...
	function addCategory() {
		...
		onquerychange(
			data.concat([
				{ Category: "Category" + ++cats.length, Keyword: "" },
			]),
		);
	}
</script>
...
{#each categories as category (category)}
	<div class="category">
		<CategoryInput
			colour={colour(category)}
			value={category}
			oncommit={handleCategoryEdit}
		/>
		<CategorySelect
			{category}
...
{/each}
<button type="button" class="chip preset-tonal w-fit" onclick={addCategory}
	>+ Add category</button
>
